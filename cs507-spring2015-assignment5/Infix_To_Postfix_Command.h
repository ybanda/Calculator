...
*/
	...
	//
	bool evaluate_postfix(Expr_Command_Iterator<Expr_Command *> & iter, int & result , bool isFlyWeight);



	bool infix_to_postfix(const std::string & infix,Expr_Command_Factory & factory,Array <Expr_Command *> & postfix, bool isFlyWeight );

	//
	//Clear Elements after QUIT
	//
	bool finally_clearCommands(Expr_Command_Iterator<Expr_Command *> & iterator);

	//bool evaluate_postfix_flyweight (Expr_Command_Iterator<Expr_Command *> & iterator, int & result) ;
